...
// this will be the (mostly) unique client ID
let CID = Date.now();
...
let URL_WINSOCK             = `ws://${URL_LOCATION}:${URL_PORT}/ws/${CID}`;
let URL_GET_TILESETS        = `${URL_SERVER}/tiles/tilesets`;
let URL_GET_TILE            = `${URL_SERVER}/tiles/#DBFILE#/{z}/{x}/{-y}.#FMT#`;
...
let URL_GET_DATAFILES       = `${URL_SERVER}/getdatafiles/${CID}`;
let URL_GET_AIRPORTS        = `${URL_SERVER}/getairports/${CID}`;
 ...
 $.get({
    ...
    success: (data) => {
        try {
            ...
            currentZoom = config.startupzoom;
        }
        catch(err) {
            console.log(err);
        }
 ...
 */
 $(() => { 
    try {
        ...
        websock = new WebSocket(URL_WINSOCK);
...
let tafMarker = new ol.style.Icon({
    ...
    src: `${URL_SERVER}/static/img/taf.svg`,
    size: [126, 90],
    offset: [0, 0],
    opacity: 1,
    scale: .25
...
let airportMarker = new ol.style.Icon({
    crossOrigin: 'anonymous',
    src: `${URL_SERVER}/static/img/dot.png`,
    size: [55, 55],
    offset: [0, 0],
    opacity: 1,
    scale: .30
...
let heliportMarker = new ol.style.Icon({
    crossOrigin: 'anonymous',
    src: `${URL_SERVER}/static/img/helipad.png`,
    size: [55, 55],
    offset: [0, 0],
    opacity: 1,
    scale: .50
...
let timerId = 0;
function keepAlive() { 
    var timeout = config.keepaliveintervalmsec;  
    if (wsOpen) {  
...
airplaneElement.src = `/static/img/${config.ownshipimage}`;
 ...
 */
 function displayMetarPopup(feature) {
    ...
    let rawmetar = metar["RawText"];
    let ident = metar.StationId;
    let svg = feature.get("svgimage");
    let cat = metar.FlightCategory;
    if (cat == undefined || cat == "undefined" || cat == ""){
        cat = "VFR";
    }
    let time = metar.ObservationTime;
    if (config.uselocaltime) {
        time = getLocalTime(time);
    }
    let tempC = metar.TempC;
    let dewpC = metar.DewpointC;
    let temp = convertCtoF(metar.TempC);
    let dewp = convertCtoF(metar.DewpointC);
    let windir = ""; 
    if (metar.WindDirDegrees === undefined || metar.WindDirDegrees === "undefined" || metar.WindDirDegrees == "") {
        windir = "Not reported";
    }
    else {
        windir = `${metar.WindDirDegrees}°`;
    }
    let winspd = metar.WindSpeedKt + "";
    let wingst = metar.WindGustKt + ""; 
    let altim = getAltimeterSetting(metar.AltimInHg);
    let vis = "";
    if (metar.VisibilityStatuteMi == null ||
        metar.VisibilityStatuteMi == undefined || 
        metar.VisibilityStatuteMi == "undefined" || 
        metar.VisibilityStatuteMi == "") {
        vis = "Not reported"
    }
    else {
        vis = getDistanceUnits(metar.VisibilityStatuteMi);
    }
    let wxcode = "";
    if (metar.WxString !== null  && 
        metar.WxString !== undefined && 
        metar.WxString !== "") {
        wxcode = decodeWxDescriptions(metar.WxString);
    }
    let taflabelcssClass = "taflabel"
    let skycondition = metar.SkyCondition;
    let skyconditions;
    let icingconditions;
    ...
    let icingcondition = metar.IcingCondition;
    if (icingcondition !== null && 
        icingcondition !== undefined && 
        icingcondition !== "") {
...
function displayTafPopup(feature) {
    ...
    let rawtaf = taf["RawText"];
    let forecasts = taf.Forecast;
    ...
    Object.keys(forecasts).forEach(index => {
        ...
        Object.keys(forecast).forEach(field => {
            let value = forecast[field]; 
            if (value !== null &&
                value !== undefined &&
                value !== "") {
                switch (field) {
                    ...
                    case "WindSpeedKt":
                    case "WindGustKt":
                    case "WindShearHgtFtAgl":
                    case "WindShearDirDegrees":
                    case "WindShearSpeedKt":
                    case "VertVisFt":    
                    case "SfcTempC":
                    case "MaxTempC":
                    case "MinTempC":
                        if (value !== 0) {
                            html += parseForecastField(field, value);
                        }
                        break;
                    case "AltimInHg":
                        if (value !== 0) {
                            let altim = getAltimeterSetting(value);
                            html += parseForecastField(field, altim);
                        }
                        break;
                    case "FcstTimeFrom":
                    case "FcstTimeTo":
                    ...
                    case "ValidTime":
                        ...
                        break;
                    case "Temperature":
                    case "VisibilityStatuteMi":
                        // if (value !== null){
                        //     value.forEach((temp) => {
                        //         html += parseForecastField(field, temp);
                        //     })
                        // }
                        break;
                    case "SkyCondition":
                        html += decodeSkyCondition(value);
                        break;
                    case "TurbulenceCondition":
                    case "IcingCondition":
                        html += decodeIcingOrTurbulenceCondition(value);
                        //html += parseForecastField(field, value);
                        break;
                    default:
                        break;
...
function parseForecastField(rawfieldname, fieldvalue) {
    ...
    switch (rawfieldname) {
        case "FcstTimeFrom":
            ...
            break;
        case "FcstTimeTo": // I'm going to ignore this field to save space on the popup
        ...
        case "Weather":
        case "VisibilityStatuteMi":
            if (fieldname === "WxString" || fieldname === "Weather") {
 ...
 */
 function displayPirepPopup(feature) {
    ...
    let rawpirep = pirep["RawText"];
    let outerhtml = `<div class="taftitle">` + 
                        `<label class="taftitlelabel">${pirep.PirepType} FROM AIRCRAFT: ${pirep.AircraftRef}</label><p></p>` +
                    `</div>` +
                    `<div class="pirep">` + 
                        `<pre><code>` +
                        `<table class="tafmessage" id="taftable">` +
    ...
    for (const pirepkey in pirep) {
        ...
        if (pirepvalue !== null && pirepvalue !== "" && pirepvalue !== undefined) {
 ...
 */
 function decodeSkyCondition(skycondition, labelclassCss) {
    let html = "";
    if (skycondition !== "" && skycondition !== undefined) {
        try {
                    ...
                    for (const index in condition) {
                        if (index != "Text") {
                            ...
                            if (fieldvalue !== "") {
                                let decodedvalue = "";
                                if (isNaN(fieldvalue)) {
                                    decodedvalue = skyConditionKeymap.get(fieldvalue);    
                                }
                                else {
                                    decodedvalue = fieldvalue;
                                } 
                                html += `<label class="${labelclassCss}">${fieldname}: <b>${decodedvalue}</b></label><br />`;
                            }
                        }
                    }
                }
    ...
    for (const item in condition) {
        ...
        if (typeof(value) === 'object') {
            ...
            for (const subitem in value) {
                if (subitem !== "Text" && subitem !== undefined) {
...
function parseConditionField(rawfieldname, fieldvalue) {
    ...
    switch (rawfieldname) {
        ...
        case "IcingIntensity":
            ...
            break;FROM
        case "TurbulenceBaseFtMsl":
        case "IcingBaseFtMsl":
            html += `<label class="pirepitem">${fieldname}: <b>${fieldvalue}</b></label><br />`;
            break;
        case "TurbulenceTopFtMsl":
        case "IcingTopFtMsl":
            html += `<label class="pirepitem">${fieldname}: <b>${fieldvalue}</b></label></br />`;
            break;
        default:
 ...
 */
 function processMetars(metarsobject) {
    let newmetars = metarsobject.metars;
    if (newmetars !== undefined) {
        ...
        try {
            newmetars.forEach((metar) => {
                ...
                let windbarbsvg = "";
                try { 
                    popupsvg = rawMetarToSVG(metar.RawText, 150, 150, config.usemetricunits);
                ...
                metarFeature.setId(metar.StationId);
    ...
    if (newtafs !== undefined) {
        ...
        try {
            newtafs.forEach((taf) => {
                ...
                taffeature.setId(taf.StationId);
 ...
 */
 function processPireps(pirepsobject) {
    let newpireps = pirepsobject.pireps;
    if (newpireps !== undefined) {
        ...
        try {
            newpireps.forEach((pirep) => {
                ...
                if (pirep.WindDirDegrees) {
                    heading = (pirep.WindDirDegrees * 0.0174533);
                }
                let pirepfeature = new ol.Feature({
                    ident: pirep.AircraftRef,
                    pirep: pirep,
                    datatype: "pirep",
                    geometry: new ol.geom.Point(ol.proj.fromLonLat([pirep.Longitude, pirep.Latitude])),
                });
                
                pirepfeature.setId(pirep.AircraftRef);
                pirepfeature.setStyle(new ol.style.Style({
                                        image: new ol.style.Icon({
                                            crossOrigin: 'anonymous',
                                            src: `${URL_SERVER}/static/img/airplane.svg`,
                                            //size:[85, 85],
                                            offset: [0,0],
                                            opacity: 1,
                                            scale: .05,
...
$.get(`${URL_GET_TILESETS}`, (data) => {
    ...
    Object.keys(meta).forEach((key) => {
        let bounds = meta[key]["bounds"];
        let minzoom = meta[key]["minzoom"]; 
        let maxzoom = meta[key]["maxzoom"]; 
        let format = meta[key]["format"];
        minzoom = minzoom ? parseInt(minzoom) : 1;
        maxzoom = maxzoom ? parseInt(maxzoom) : 18;
        let url = URL_GET_TILE.replace("#DBFILE#", key).replace("#FMT#", format);
        let title = key.replace(".mbtiles", "");
        let ext = [-180, -85, 180, 85];
        if (bounds) {
            ext = bounds.split(',').map(Number)
        }
 ...
 */
if (config.savepositionhistory) {
    setInterval(savePositionHistory, config.histintervalmsec);
}
 ...
 */
if (config.getgpsfromstratux) {
    setInterval(getGpsData, config.gpsintervalmsec);
}
 ...
 */
function getGpsData() {
    $.get(config.stratuxurl, function(data) {
...
function loadTafFieldKeymap() {
    tafFieldKeymap.set("TempC", "Temperature °C");
    tafFieldKeymap.set("IcingType", "Icing type");
    tafFieldKeymap.set("PirepType", "Pirep type");
    tafFieldKeymap.set("AltitudeFtMSL", "Altitude in feet MSL");
    tafFieldKeymap.set("AltitudeFtMsl", "Altitude in feet MSL");
    tafFieldKeymap.set("ReceiptTime", "Receipt time")
    tafFieldKeymap.set("ObservationTime", "Observation time")
    tafFieldKeymap.set("Latitude", "Latitude")
    tafFieldKeymap.set("Longitude", "Longitude")
    tafFieldKeymap.set("CloudType", "Cloud type");
    tafFieldKeymap.set("FcstTimeFrom", "Time from");
    tafFieldKeymap.set("FcstTimeTo", "Time to");
    tafFieldKeymap.set("ChangeIndicator", "Change indicator");
    tafFieldKeymap.set("TimeBecoming", "Time becoming");
    tafFieldKeymap.set("Probability", "Probability");
    tafFieldKeymap.set("WindDirDegrees", "Wind Direction");
    tafFieldKeymap.set("WindSpeedKt", "Wind Speed knots");
    tafFieldKeymap.set("WindGustKt", "Wind Gust knots");
    tafFieldKeymap.set("WindShearHgtFtAGL", "Shear height feet AGL");
    tafFieldKeymap.set("WindShearHgtFtAgl", "Shear height feet AGL");
    tafFieldKeymap.set("WindShearDirDegrees", "Shear direction");
    tafFieldKeymap.set("WindShearSpeedKt", "Shear speed knots");
    tafFieldKeymap.set("AltimInHg", "Altimeter (Hg)");
    tafFieldKeymap.set("VertVisFt", "Vertical visibility in feet");
    tafFieldKeymap.set("VisibilityStatuteMi", "Horizontal visibility in statute miles");
    tafFieldKeymap.set("WxString", "Weather");
    tafFieldKeymap.set("SkyCondition", "Sky condition");
    tafFieldKeymap.set("IcingCondition", "Icing condition");
    tafFieldKeymap.set("TurbulenceCondition", "Turbulence condition");
    tafFieldKeymap.set("SkyCover", "Sky cover");
    tafFieldKeymap.set("CloudBaseFtAGL", "Cloud base feet AGL");
    tafFieldKeymap.set("CloudBaseFtMSL", "Cloud base feet MSL");
    tafFieldKeymap.set("CloudBaseFtAgl", "Cloud base feet AGL");
    tafFieldKeymap.set("CloudBaseFtMsl", "Cloud base feet MSL");
    tafFieldKeymap.set("CloudBase", "Cloud base");
    // icing fieldnames
    tafFieldKeymap.set("IcingIntensity", "Intensity");
    tafFieldKeymap.set("IcingType", "Type");
    
    tafFieldKeymap.set("IcingMinAltFtAGL", "Min altitude feet AGL");
    tafFieldKeymap.set("IcingMaxAltFtAGL", "Max altitude feet AGL");
    tafFieldKeymap.set("IcingMinAltFtMSL", "Min altitude feet MSL");
    tafFieldKeymap.set("IcingMaxAltFtAGL", "Max altitude feet MSL");
    tafFieldKeymap.set("IcingTopFtMSL", "Top in feet MSL");
    tafFieldKeymap.set("IcingBaseFtMSL", "Base in feet MSL");
    ...
    tafFieldKeymap.set("TurbulenceFreq", "Frequency");
    tafFieldKeymap.set("TurbulenceType", "Type");
    tafFieldKeymap.set("TurbulenceIntensity", "Intensity");
    
    tafFieldKeymap.set("TurbulenceMinAltFtAGL", "Min altitude feet AGL");
    tafFieldKeymap.set("TurbulenceMaxAltFtAGL", "Max altitude feet AGL");
    tafFieldKeymap.set("TurbulenceTopFtMSL", "Top in feet MSL");
    tafFieldKeymap.set("TurbulenceBaseFtMSL", "Base in feet MSL");
 ...
 */
 function loadMetarFieldKeymap() {
    metarFieldKeymap.set("ChangeIndicator", "Change indicator");
    metarFieldKeymap.set("RawText", "raw text");
    metarFieldKeymap.set("StationId", "StationId"); 
    metarFieldKeymap.set("ObservationTime", "Observation Time");
    metarFieldKeymap.set("Latitude", "latitude");
    metarFieldKeymap.set("Longitude", "longitude");
    metarFieldKeymap.set("TempC", "Temp °C");
    metarFieldKeymap.set("DewpointC", "Dewpoint °C");
    metarFieldKeymap.set("WindDirDegrees", "Wind direction"); 
    metarFieldKeymap.set("WindSpeedKt", "Wind speed knots");
    metarFieldKeymap.set("WindGustKt", "Wind gust knots");
    metarFieldKeymap.set("VisibilityStatuteMi", "Horizontal visibility in statute miles");
    metarFieldKeymap.set("AltimInHg", "Altimeter in Hg");
    metarFieldKeymap.set("SeaLevelPressureMb", "Sea-level pressure in MB");
    metarFieldKeymap.set("QualityControlFlags", "Quality control flags");
    metarFieldKeymap.set("WxString", "Weather");
    metarFieldKeymap.set("SkyCondition", "Sky cover");
    metarFieldKeymap.set("SkyCover", "Sky cover");
    metarFieldKeymap.set("CloudBaseFtAGL", "Cloud base feet AGL");
    metarFieldKeymap.set("CloudBase", "Cloud base");
    metarFieldKeymap.set("FlightCategory", "Flight category");
    metarFieldKeymap.set("ThreeHrPressureTendencyMb", "Pressure change past 3 hours in MB");
    metarFieldKeymap.set("MaxTC", "Max air temp °C, past 6 hours");
    metarFieldKeymap.set("MinTC", "Min air temp °C, past 6 hours");
    metarFieldKeymap.set("MaxT24HrC", "Max air temp °C, past 24 hours");
    metarFieldKeymap.set("MinT24HrC", "Min air temp °C, past 24 hours");
    metarFieldKeymap.set("PrecipIn", "Liquid precipitation since last METAR");
    metarFieldKeymap.set("Pcp3hrIn", "Liquid precipitation past 3 hours");
    metarFieldKeymap.set("Pcp6hrIn", "Liquid precipitation past 6 hours");
    metarFieldKeymap.set("Pcp24hrIn", "Liquid precipitation past 24 hours");
    metarFieldKeymap.set("SnowIn", "Snow depth in inches");
    metarFieldKeymap.set("VertVisFt", "Vertical visibility in feet");
    metarFieldKeymap.set("MetarType", "Metar type");
    metarFieldKeymap.set("ElevationM", "Station elevation in meters");
}
...
function loadWeatherAcronymKeymap() {
    ...
    weatherAcronymKeymap.set("DZ", "Drizzle");
    weatherAcronymKeymap.set("FZDZ", "Freezing drizzle");
    weatherAcronymKeymap.set("DZRA", "Drizzle and rain");
    weatherAcronymKeymap.set("RA", "Rain");
    weatherAcronymKeymap.set("FZRA", "Freezing rain");
    weatherAcronymKeymap.set("RASN", "Rain and snow");
    weatherAcronymKeymap.set("SN", "Snow");
    ...
    weatherAcronymKeymap.set("SHRA", "Rain showers");
    weatherAcronymKeymap.set("SHRASN", "Rain and snow showers");
    weatherAcronymKeymap.set("SHSN", "Snow showers");
    weatherAcronymKeymap.set("GR", "Showers with hail, not with thunder");
    weatherAcronymKeymap.set("TSRA", "Thunderstorm with rain");
    weatherAcronymKeymap.set("TSGR", "Thunderstorm with hail");
 ...
 */
 function loadSkyConditionmKeymap() {
    ...
    skyConditionKeymap.set("CLR", "Clear");
    skyConditionKeymap.set("FEW", "Few clouds");
 ...
 */
 function decodeWxDescriptions(codevalue) {
    ...
    for (let i = 0; i < vals.length; i++) {
        ...
        if (i === 0) {
            outstr = `${modifier}${weatherAcronymKeymap.get(wx)}`;
        }
        else {
            outstr += ` / ${modifier}${weatherAcronymKeymap.get(wx)}`;
        }
...
function getWindBarbSvg(width, height, metar) {
    ...
    let svg = "";
    let thismetar = {
        wind_direction: metar.WindDirDegrees,
        wind_speed: metar.WindSpeedKt,
        gust_speed: metar.GustSpeedKt,
